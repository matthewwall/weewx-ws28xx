#!/usr/bin/python
# $Id: ws28xx_lheijst2.py 2357 2014-09-28 12:17:35Z mwall $
#
...
class CCommunicationService(object):
    ...
    def __init__(self, cfgfn):
        ...
        self.CpuCorr = 0.0030# duration of idle loop (depend on cpu speed)
        self.Period1 = 0.095 # period since StartTime (catch WS time set message)
        self.Period2 = 0.365 # period since StartTime (catch history or 06 message)
        self.Period3 = self.DataStore.getCommModeInterval()+1 -0.020 # to catch current weather message
        self.durLoop1= 0.270 # duration of loop1 
    ...
    def handleHistoryData(self,Buffer,Length):
        ...
        if ThisHistoryIndex == LatestHistoryIndex:
            ...
            if abs(self.TimeDifSec) >= MaxTimeDifference:
                ...
                logdbg('m_Time          %s      WS clock offset %4s s' % (Data.m_Time, self.TimeDifSec))
                self.DataStore.setRequestType(ERequestType.rtINVALID)
        else:
...
##here should fail the linux first config .... our tranceiverid is tored thru ack frame
    ...
    def doRFCommunication(self):
        ...
        if ret == 1:
            ...
            if ReceiverState == 0x16:
                try:
                    ...
                    if self.WaitSteps >= 5:
                        self.CpuCorr = (TotalLoopTimeSecs - self.TotalSleepTime) / self.WaitSteps
                        if self.CpuCorr > 0.0090:
                            self.CpuCorr = 0.0030
                    logdbg('WaitSteps %4s, TotalSleepTime %6.3f, TotalLoopTime %6.3f, CpuCorr %7.4f' % (self.WaitSteps, self.TotalSleepTime, TotalLoopTimeSecs, self.CpuCorr))
        ...
        # Calculate each loop the optimal WaitTime
        self.WaitSteps += 1
        endLoop1 = self.Period1 + self.Period2 + self.durLoop1 - self.WaitSteps*self.CpuCorr
        try:
            TotalLoopTime = (datetime.now() - self.StartTime)
            TotalLoopTimeSecs = TotalLoopTime.seconds + TotalLoopTime.microseconds/1000000.0
        except:
            logerr('WaitTime - Server time set in future')
            self.StartTime = datetime.now()
            self.WaitSteps = 1 
            TotalLoopTimeSecs = 0
        if self.WaitSteps == 1:
            self.WaitTime = self.Period1 
        elif self.WaitSteps == 2:
            self.WaitTime = self.Period2 - self.Period1 - self.CpuCorr
        elif (self.WaitSteps >= 3 and TotalLoopTimeSecs < endLoop1):
            self.WaitTime = self.slpLoop1  # loop1
        elif (TotalLoopTimeSecs >= endLoop1 and TotalLoopTimeSecs < (endLoop1+0.5)):
            self.WaitTime = self.Period3 - endLoop1
        else:
            self.WaitTime = self.slpLoop2 # loop2
        self.TotalSleepTime += self.WaitTime
        logdbg('sleep %6.3f' % self.WaitTime)
        time.sleep(self.WaitTime) # self.WaitTime #0.005
