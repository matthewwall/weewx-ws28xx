#!/usr/bin/python
# $Id: ws28xx-0.29-lh-0.1.py 2357 2014-09-28 12:17:35Z mwall $
#
...
class WS28xx(weewx.abstractstation.AbstractStation):
    ...
    def genStartupRecords(self, ts):
        ...
        # When ts=0 handleHistorydata will know the caching request came from
        # the driver itself and not from wee_config_ws28xx
        ts = 0 
        maxtries = 3
        ntries = 0
        last_n = n = nrem = None
...
class CCommunicationService(object):
    ...
    def handleHistoryData(self, buf, buflen):
        ...
        filename = '/tmp/ws28xx-history.tmp'
        if self.command == EAction.aGetHistory:
            if self.history_cache.start_index is None:
                ...
                if (last_idx is None or
                    ...
                    last_since_ts = last_ts
                logdbg('handleHistoryData: using last_idx=%s last_ts=%s' %
                       (last_idx, last_ts))
                nrec_ws = nrec + 1
                if self.history_cache.num_rec > 0:
                    logdbg('handleHistoryData: wee_config_ws28xx request for %s records' %
                    ...
                    logdbg('handleHistoryData: wee_config_ws28xx request records since %s' %
                           weeutil.weeutil.timestamp_to_string(self.history_cache.since_ts))
                    history_period = last_since_ts - ts
                    ...
                    logdbg('handleHistoryData: ws28xx-driver itself request catchup')
                    idx = last_idx
