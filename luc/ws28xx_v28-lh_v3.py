#!/usr/bin/python
# $Id: ws28xx_v28-lh_v3.py 2357 2014-09-28 12:17:35Z mwall $
#
...
DRIVER_VERSION = '0.28 modified by lh - version 0.3'
...
class CCommunicationService(object):
    ...
    def buildACKFrame(self, Buffer, action, cs, hidx=None):
        ...
        if self.command == EAction.aGetHistory:
            ...
            if action == EAction.aGetHistory and age >= comInt * 2 and newBuffer[0][1] != 0xF0:
                if DEBUG_COMM > 0:
        ...
        if self.command == EAction.aGetHistory:
            if self.history_cache.start_index is None:
                ...
                try:  
                    ...
                    logdbg('handleHistoryData: could not read LatestHistory; read all records')
                    last_idx = thisIndex	# get eldest init history record instead
                    last_since_ts = tstr_to_ts(str(data.Time))
                ...
                if self.history_cache.num_rec > 0:
                    ...
                    if history_period <= 0 or nrec <= 0:
                        logdbg('handleHistoryData: wrong parameters, history_period: %s, nrec: %s; read latest record' % (history_period,nrec))
                        idx = latestIndex	# get latest init history record instead
                    else:
                        history_interval = history_period / nrec
                ...
                if nrec < 1795 and idx > latestIndex:
