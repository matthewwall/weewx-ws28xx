#!/usr/bin/python
# $Id: ws28xx_lheijst.py 2357 2014-09-28 12:17:35Z mwall $
#
    ...
    def ToDateTime(buf, start, startOnLowNibble):
        if ( USBHardware.IsErr2(buf, start+0, startOnLowNibble)
            ...
            try:
                ...
                logerr('Error in date timeformat %4i-%2i-%2i %2i:%2i' % (year, month, days, hours, minutes))
                result = datetime(1900, 01, 01, 00, 00)
        return result
...
class CDataStore(object):
    ...
    class TSettings(object):
        def __init__(self):
            self.CommModeInterval = 4 #lh was: 3
            self.DeviceId = -1
...
#CScopedLock::_CScopedLock(&lock);
    ...
    def setLastCurrentWeatherTime(self,time):
        logerr("setLastCurrentWeatherTime: time=%s" % (time))
...
class CCommunicationService(object):
    ...
    def __init__(self, cfgfn):
        ...
        self.Period1 = 0.095 # period since StartTime (catch WS time set message)
        self.Period2 = 0.465 # period since StartTime (catch history or 06 message)
        self.Period3 = self.DataStore.getCommModeInterval() + 1 # to catch current weather message
        self.durLoop1= 0.200 #0.350 # duration of loop1 
        self.slpLoop1= 0.001 # sleeptime of loop1 (catch history message) 
        self.slpLoop2= 0.100 # sleeptime of loop2 (catch current weather message)   
...
#			}
    ...
    def handleHistoryData(self,Buffer,Length):
        ...
        #logerr('LatestHistoryAddres %x, LatestHistoryIndex %i' % (LatestHistoryAddres, LatestHistoryIndex))
        #logerr('ThisHistoryAddres   %x, ThisHistoryIndex   %i' % (ThisHistoryAddres, ThisHistoryIndex))
        #v6 = CTracer::Instance();
        ...
        if ( LatestHistoryIndex >= ThisHistoryIndex ): #unused
            DifHis = LatestHistoryIndex - ThisHistoryIndex
            #self.DataStore.setOutstandingHistorySets( self.DifHis) #unused
            if DifHis > 0:
                logerr('m_Time          %s  OutstandingHistorySets: %4i' % (Data.m_Time, DifHis))
        else:
        ...
        if ThisHistoryIndex == LatestHistoryIndex:
            ...
            TimeDifSec = (Data.m_Time - datetime.now()).seconds
            if TimeDifSec > 43200:
                TimeDifSec = TimeDifSec - 86400 +1
            if abs(TimeDifSec) >= MaxTimeDifference:
                logerr('m_Time          %s  abs(WS clock offset %4s s) >= %i s; set RequestType=rtSetTime' % (Data.m_Time, TimeDifSec, MaxTimeDifference))
                self.DataStore.setRequestType(ERequestType.rtSetTime)
            else:
                logerr('m_Time          %s      WS clock offset %4s s' % (Data.m_Time, TimeDifSec))
                self.DataStore.setRequestType(ERequestType.rtINVALID)
        else:
        ...
        DeviceCS = self.DataStore.GetDeviceConfigCS()
        if   rt == ERequestType.rtGetCurrent: #rtGetCurrent
            #lh 'Buffer' changed to 'newBuffer' (6 times)
...
#            newLength[0] = self.buildConfigFrame(newBuffer, v16);
            ...
            if (Buffer[0][2] & 0xF) == 3: #(CWeatherStationConfig *)
                logerr('WS SetTime - Request Time')
    ...
    def GenerateResponse(self,Buffer,Length):
        ...
        if Length[0] != 0:
            ...
            logdbg("GenerateResponse: Length=%x RequestType=%x" % (Length[0], RequestType))
...
##here should fail the linux first config .... our tranceiverid is tored thru ack frame
    ...
    def doRF(self):
        try:
            logerr('Initializing rf communication')
            logerr('period1 =%6.3f period2 =%6.3f period3 =%6.3f' % (self.Period1, self.Period2, self.Period3))
            logerr('durloop1=%6.3f slploop1=%6.3f slploop2=%6.3f' % (self.durLoop1, self.slpLoop1, self.slpLoop2))
    ...
    def doRFCommunication(self):
        ...
        if ret == 1:
            ...
            if ReceiverState == 0x16:
                try:
                    TotalLoopTime = (datetime.now() - self.StartTime)
                    TotalLoopTimeSecs = TotalLoopTime.seconds + TotalLoopTime.microseconds/1000000.0
                    ###logerr('WaitSteps %5s, TotalSleepTime %6.3f  TotalLoopTime %6.3f' % (self.WaitSteps, self.TotalSleepTime, TotalLoopTimeSecs))
                    ...
                    logerr('ReceiverState 0x16 - Server time set in future')
                self.TotalSleepTime = 0 # Reset TotalSleepTime after handling a 0x16 message
                self.WaitSteps = 0      # Informational: number of wait loops before next 0x16 message
                self.StartTime = datetime.now()
        ...
        # Calculate each loop the optimal SleepTime
        try:
            TotalLoopTime = (datetime.now() - self.StartTime)
            TotalLoopTimeSecs = TotalLoopTime.seconds + TotalLoopTime.microseconds/1000000.0
        except:
            logerr('TotalWaitTime - Server time set in future')
            self.StartTime = datetime.now()
            self.WaitSteps = 1 
            TotalLoopTimeSecs = 0
        self.WaitSteps += 1
        endLoop1 = self.Period1 + self.Period2 + self.durLoop1
        if self.WaitSteps == 1:
            self.SleepTime = self.Period1 
        elif self.WaitSteps == 2:
            self.SleepTime = self.Period2 - self.Period1
        elif (self.WaitSteps >= 3 and TotalLoopTimeSecs < endLoop1):
            self.SleepTime = self.slpLoop1  # loop1
        elif (TotalLoopTimeSecs >= endLoop1 and TotalLoopTimeSecs < (endLoop1+0.5)):
            self.SleepTime = self.Period3 - endLoop1
        else:
            self.SleepTime = self.slpLoop2 # loop2
        self.TotalSleepTime += self.SleepTime
        logdbg('sleep %6.3f' % self.SleepTime)
