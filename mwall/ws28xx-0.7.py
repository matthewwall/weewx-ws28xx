#!/usr/bin/python
# $Id: ws28xx-0.7.py 2357 2014-09-28 12:17:35Z mwall $
#
...
# FIXME: make rt clauses for buildACKFrame the same?
...
class WS28xx(weewx.abstractstation.AbstractStation):
    ...
    def genLoopPackets(self):
        ...
        while True:
            ...
            except KeyboardInterrupt, weewx.Terminate:
                self.shutdown()
                raise
            except Exception, e:
...
class CCurrentWeatherData(object):
    ...
    def _calcChecksum(self, buf, pos):
        cs = 0
        for i in xrange(0, len(buf[0])-pos):
            cs += buf[0][i+pos]
        return cs
    ...
    def read(self, buf, pos):
        ...
        self._checksum = self._calcChecksum(buf, pos)
...
class CWeatherStationConfig(object):
    def __init__(self, cache_file):
        ...
        if self.cache_file is not None:
            ...
            try:
                self._DeviceCS = int(config['Station']['DeviceCS'])
            except:
                pass
    ...
    def write(self):
        ...
        if DEBUG_WRITES > 0:
            logdbg('WeatherStationConfig_write: write to %s' % self.cache_file)
        config.write()
    ...
    def read(self,buf,pos):
        ...
        self._HistoryInterval = EHistoryInterval.hi60Min # hi24Std hi01Min
    ...
    def testConfigChanged(self,buf):
        ...
        if (self._OutBufCS == self._InBufCS) and (self._ResetMinMaxFlags  == 0):
            ...
            logdbg('testConfigChanged: Checksum or resetMinMaxFlags changed, InBufCS=%04x, OutBufCS=%04x, _ResetMinMaxFlags=%06x' % (self._InBufCS, self._OutBufCS, self._ResetMinMaxFlags))
            if DEBUG_CONFIG_DATA > 0:
                self.logConfigData()
            self.write()
            State = 1
...
class CHistoryDataSet(object):
    ...
    def read(self, buf, pos):
        ...
        logdbg("WindDirection=        %.3s" % CWeatherTraits.windDirMap[self.m_WindDirection])
...
class CDataStore(object):
    ...
    class TLastStat(object):
        def __init__(self, cache_file):
            ...
            if cache_file is not None:
                config = ConfigObj(cache_file)
                config.filename = cache_file
                try:
    ...
    def __init__(self, cache_file):
        ...
        self.DeviceConfig = CWeatherStationConfig(cache_file)
...
#    00000000: d5 00 09 00 32 00 04 cf 00 30 01 a0 00 00 00 00
    ...
    # filter output that we do not care about
...
class CCommunicationService(object):
    ...
    def buildTimeFrame(self,Buffer,deviceCS,checkMinuteOverflow):
        logdbg("buildTimeFrame: deviceCS=%x checkMinuteOverflow=%s" % 
        ...
        if ( checkMinuteOverflow and (sec <= 5 or sec >= 55) ):
            ...
            Length = self.buildACKFrame(newBuffer, 5, deviceCS, idx, sec)
            Buffer[0]=newBuffer[0]
        else:
            #00000000: d5 00 0c 00 32 c0 00 8f 45 25 15 91 31 20 01 00
            #00000000: d5 00 0c 00 32 c0 06 c1 47 25 15 91 31 20 01 00
    ...
    def buildACKFrame(self,Buffer, action, deviceCS, historyIndex, comInt):
        logdbg("action=%x deviceCS=%x historyIndex=%x comInt=%x" %
...
#                self.DataStore.setBufferCheck( 2)
    ...
    def handleConfig(self,Buffer,Length):
        ...
        # FIXME: rt will always be rtGetCurrent?
    ...
    def handleCurrentData(self,Buffer,Length):
        ...
        if (age >= timedelta(seconds=10)
            or data.checksum() != self.DataStore.CurrentWeather.checksum()):
        ...
        idx = self.DataStore.getLastHistoryIndex()
        rt = self.DataStore.getRequestType()
        if rt == ERequestType.rtGetConfig:
            newLength[0] = self.buildACKFrame(newBuffer,3, cs, idx, 0xFFFFFFFF)
        elif rt == ERequestType.rtSetConfig:
            newLength[0] = self.buildACKFrame(newBuffer,2, cs, idx, 0xFFFFFFFF)
...
#            newLength[0] = self.buildACKFrame(newBuffer,4, cs, idx, 0xFFFFFFFF)
    ...
    def handleHistoryData(self,Buffer,Length):
        ...
        if ThisHistoryIndex == LatestHistoryIndex:
            ...
            logdbg('handleHistoryData: timeDifSec=%4s m_Time=%s' %
                   (self.TimeDifSec, Data.m_Time))
        else:
...
#            newLength[0] = self.buildConfigFrame(newBuffer, v16)
        ...
        if Length[0] != 0:
            ...
            if bufferID == 0xF0F0:
                ...
                if responseType == 0x20:
                    ...
                    if Length[0] == 0x06:
                        loginf('generateResponse: WS SetTime/SetConfig data written')
    ...
    def transceiverIsPresent(self):
        return self.DataStore.getFlag_FLAG_TRANSCEIVER_PRESENT()
