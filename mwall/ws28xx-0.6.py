#!/usr/bin/python
# $Id: ws28xx-0.6.py 2357 2014-09-28 12:17:35Z mwall $
#
...
class sHID(object):
    ...
    def setTX(self):
        ...
        buf[0] = 0xD1
        if DEBUG_COMM > 0:
            self.dump('setTX', buf, fmt=DUMP_FORMAT)
        try:
    ...
    def setRX(self):
        ...
        buf[0] = 0xD0
        if DEBUG_COMM > 0:
            self.dump('setRX', buf, fmt=DUMP_FORMAT)
        try:
            self.devh.controlMsg(usb.TYPE_CLASS + usb.RECIP_INTERFACE,
                                 request=0x0000009,
                                 buffer=buf,
    ...
    def getState(self,StateBuffer):
        try:
            buf = self.devh.controlMsg(requestType=usb.TYPE_CLASS |
                                       ...
                                       timeout=self.timeout)
            if DEBUG_COMM > 0:
                self.dump('getState', buf, fmt=DUMP_FORMAT)
            StateBuffer[0]=[0]*0x2
    ...
    def readConfigFlash(self,addr,numBytes,data):
        if numBytes <= 512:
            while ( numBytes ):
                ...
                buf[3] = (addr >>0)  & 0xFF
                if DEBUG_COMM > 0:
                    self.dump('readCfgFlash>', buf, fmt=DUMP_FORMAT)
                try:
                    # FIXME: check return value
                    self.devh.controlMsg(usb.TYPE_CLASS + usb.RECIP_INTERFACE,
                                         request=0x0000009,
    ...
    def setState(self,state):
        ...
        buf[1] = state
        if DEBUG_COMM > 0:
            self.dump('setState', buf, fmt=DUMP_FORMAT)
        try:
            self.devh.controlMsg(usb.TYPE_CLASS + usb.RECIP_INTERFACE,
                                 request=0x0000009,
                                 buffer=buf,
...
#    00000000: d5 00 09 00 32 00 04 cf 00 30 01 a0 00 00 00 00
        ...
        for i in xrange(0, numBytes):
            buf[i+3] = data[i]
        if DEBUG_COMM > 0:
            self.dump('setFrame', buf, fmt=DUMP_FORMAT)
        try:
            self.devh.controlMsg(usb.TYPE_CLASS + usb.RECIP_INTERFACE,
                                 request=0x0000009,
                                 buffer=buf,
    ...
    def getFrame(self,data,numBytes):
        try:
            ...
            for i in xrange(0, new_numBytes):
                new_data[i] = buf[i+3]
            if DEBUG_COMM > 0:
                self.dump('getFrame', buf, fmt=DUMP_FORMAT)
            data[0] = new_data
    ...
    def execute(self,command):
        ...
        buf[1] = command
        if DEBUG_COMM > 0:
            self.dump('execute', buf, fmt=DUMP_FORMAT)
        try:
            self.devh.controlMsg(usb.TYPE_CLASS + usb.RECIP_INTERFACE,
                                 request=0x0000009,
                                 buffer=buf,
    ...
    def setPreamblePattern(self,pattern):
        ...
        buf[1] = pattern
        if DEBUG_COMM > 0:
            self.dump('setPreamPattern', buf, fmt=DUMP_FORMAT)
        try:
            self.devh.controlMsg(usb.TYPE_CLASS + usb.RECIP_INTERFACE,
                                 request=0x0000009,
                                 buffer=buf,
