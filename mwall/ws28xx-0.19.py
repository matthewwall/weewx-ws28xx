#!/usr/bin/python
# $Id: ws28xx-0.19.py 2357 2014-09-28 12:17:35Z mwall $
#
...
02: rtSetConfig  - Send Config to weather station
...
class WS28xx(weewx.abstractstation.AbstractStation):
    ...
    def __init__(self, **stn_dict) :
        ...
        self._last_nodata_log_ts = None
        self._nodata_interval = 600 # how often to log about no data, seconds
    ...
    def genLoopPackets(self):
        ...
        while True:
            ...
            if packet is None:
                now = int(time.time())
                if self._last_nodata_log_ts is None:
                    self._last_nodata_log_ts = now
                if now - self._last_nodata_log_ts > self._nodata_interval:
                    dur = now - self._last_obs_ts
                    self._last_nodata_log_ts = now
...
#        pass
    ...
    def get_config(self):
        ...
        return self._service.getConfigData().asDict()
...
class CWeatherStationConfig(object):
    ...
    def asDict(self):
        return {
            'in_checksum': self._InBufCS,
            'out_checksum': self._OutBufCS,
            'device_checksum': self._DeviceCS,
            'clock_mode': self._ClockMode,
            'temperature_format': self._TemperatureFormat,
            'pressure_format': self._PressureFormat,
            'rain_format': self._RainFormat,
            'windspeed_format': self._WindspeedFormat,
            'weather_threshold': self._WeatherThreshold,
            'storm_threshold': self._StormThreshold,
            'lcd_contrast': self._LCDContrast,
            'low_battery_flags': self._LowBatFlags,
            'wind_dir_alarm_flags': self._WindDirAlarmFlags,
            'other_alarm_flags': self._OtherAlarmFlags,
