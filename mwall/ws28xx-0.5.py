#!/usr/bin/python
# $Id: ws28xx-0.5.py 2357 2014-09-28 12:17:35Z mwall $
#
...
class CWeatherStationConfig(object):
    ...
    def testConfigChanged(self,buf):
        ...
        if (self._OutBufCS == self._InBufCS) and (self._ResetMinMaxFlags  == 0):
            ...
            logdbg('Checksum or resetMinMaxFlags changed, InBufCS=%04x, OutBufCS=%04x, _ResetMinMaxFlags=%06x' % (self._InBufCS, self._OutBufCS, self._ResetMinMaxFlags))
            self.logConfigData()
            self.write()
            State = 1
        return State
    ...
    def dump(self, cmd, buf, fmt='matrix'):
        if fmt == 'matrix':
            ...
            for i in buf:
                strbuf += str("%.2x" % i)
        if len(strbuf) > 0:
            if strbuf in ['de1500000000','de 15 00 00 00 00 ',
                          'de1400000000','de 14 00 00 00 00 ']:
...
class CCommunicationService(object):
    ...
    def buildTimeFrame(self,Buffer,checkMinuteOverflow):
        ...
        if ( checkMinuteOverflow and (sec <= 5 or sec >= 55) ):
            ...
            Length = self.buildACKFrame(newBuffer, 5, cs, idx, sec)
            Buffer[0]=newBuffer[0]
        else:
            #00000000: d5 00 0c 00 32 c0 00 8f 45 25 15 91 31 20 01 00
            #00000000: d5 00 0c 00 32 c0 06 c1 47 25 15 91 31 20 01 00
...
#                self.DataStore.setBufferCheck( 2)
    ...
    def handleCurrentData(self,Buffer,Length):
        ...
        # save Current Data only if prevous message was more than 3 seconds ago
        # FIXME: why 3 seconds?
        if now - self.DataStore.LastStat.LastCurrentWeatherTime > timedelta(seconds=3):
            logdbg('handleCurrentData: saving current weather data')
