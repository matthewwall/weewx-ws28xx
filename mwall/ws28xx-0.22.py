#!/usr/bin/python
# $Id: ws28xx-0.22.py 2357 2014-09-28 12:17:35Z mwall $
#
...
class CWeatherStationConfig(object):
    ...
    def asDict(self):
        return {
            ...
            'alarm_flags_low_battery': self._LowBatFlags,
...
class CCommunicationService(object):
    ...
    def buildACKFrame(self,Buffer, action, deviceCS, historyIndex):
        logdbg("buildACKFrame: action=%x deviceCS=%04x historyIndex=%i" %
               (action, deviceCS, historyIndex))
        newBuffer = [0]
        newBuffer[0] = [0]*9
        for i in xrange(0,2):
